...
class Smsbao
{
    ...
    public function send()
    {
        ...
        $smsapi = "http://big.smsbao.com/api/orange/sms.action";
        $url = $smsapi.'?u='.$postArr['u'].'&p='.$postArr['p'].'&m='.$postArr['m'].'&c='.urlencode($postArr['c']);
        $result = \fast\Http::get($url);
        //$result = \fast\Http::sendRequest('http://api.smsbao.com/sms', $postArr, 'GET', $options);
        $res_code = substr($result,0,1);
        if ($res_code == '0') {
            return TRUE;
        } else {
