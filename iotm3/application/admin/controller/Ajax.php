...
class Ajax extends Backend
{
    ...
    public function getSensorDetail(){
        $did = $this->request->post("did");
        $SensorLog = new \app\common\model\SensorLog;
      //  $sql = "SELECT DISTINCT  s.`label` FROM  `iot_unit_sensor` s LEFT JOIN `iot_sensor_list` l ON l.`id` = s.`sensorid` WHERE l.`did` = '{$did}' ";
          $sql = "SELECT label,id FROM `iot_sensor_list` WHERE did = '{$did}'";
        $sensorList = $SensorLog ->query($sql);
       // $labelsLog = $SensorLog ->query($sql);
        ...
        foreach ($sensorList as $k=>$v){
            ...
            foreach ($data as $s=>$val){
                ...
                $totalData[$v['id']][$hour] = $val['val'];
            }
            //7天情况(按天)
            if($v['label']!='person'){
            $sql = "select avg(val) as val from sensor_{$v['id']} where  ts < now and ts > now - 7d INTERVAL(1d) ;";
            }else{
                $sql = "select count(*) as val from sensor_{$v['id']} where ts < now and ts > now - 7d  and val = 1 INTERVAL(1d) ;";
            }
            $data = $tdengine->queryForData($sql);
            $sevenData[$v['id']] = [];
            if(!empty($data)){
            foreach ($data as $s=>$val){
                $valtime = $time->getDataFormat($val['ts']);
                $hour = $time->msecOwnDate($valtime,'d');
                $sevenData[$v['id']][intval($hour)] = $val['val'];
            }
            }else{
                $sevenData[$v['id']][0] = 0;
            }
        ...
        $list = array();
        $list['todayData'] = $totalData;
        $list['sevenData'] = $sevenData;
        return json($list);
